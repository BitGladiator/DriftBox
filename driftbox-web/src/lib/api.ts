...
const uploadClient   = createClient(UPLOAD_URL);
const metadataClient = createClient(METADATA_URL);
const shareClient    = createClient(SHARE_URL);


export const authApi = {
  signup: (email: string, password: string) =>
    authClient.post('/auth/signup', { email, password }),
...
export const uploadApi = {
  ...
  complete: (sessionId: string) =>
    uploadClient.post('/upload/complete', { sessionId }),

  status: (sessionId: string) =>
    uploadClient.get(`/upload/status/${sessionId}`),

  download: (fileId: string) =>
    uploadClient.get(`/upload/download/${fileId}`),
};


export const metadataApi = {
  listFiles: (folderPath = '/', page = 1, limit = 20) =>
    metadataClient.get('/files', { params: { folderPath, page, limit } }),

  getFile: (id: string) =>
    metadataClient.get(`/files/${id}`),

  deleteFile: (id: string) =>
    metadataClient.delete(`/files/${id}`),

  listVersions: (id: string) =>
    metadataClient.get(`/files/${id}/versions`),

  restoreVersion: (id: string, versionId: string) =>
    metadataClient.post(`/files/${id}/restore/${versionId}`),

  searchFiles: (q: string) =>
    metadataClient.get('/files/search', { params: { q } }),
};
...
export const shareApi = {
  ...
  accessLink: (linkId: string) =>
    shareClient.get(`/share/${linkId}`),

  revokeLink: (linkId: string) =>
    shareClient.delete(`/share/${linkId}`),

  myLinks: () =>
    shareClient.get('/share'),
};